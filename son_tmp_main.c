...
void main()
{
    ...
    io_init();
    setup_uart_to_boss(); // ここで割り込みがオンになる
    setup_timer();
    misf_init();
    ...
    int8 last_buffer_size = 0; // デバッグ用：前回のバッファサイズを記憶
    ...
    while(!is_finished)
    {
        ...
        // バッファのサイズが増えた（データを受信した）ら表示する
        if (boss_receive_buffer_size != last_buffer_size)
        {
            fprintf(PC, "\r\n[DEBUG] RX(%d/12): ", boss_receive_buffer_size);
            for(int i = 0; i < boss_receive_buffer_size; i++)
            {
                fprintf(PC, "%02X ", boss_receive_buffer[i]);
            }
            last_buffer_size = boss_receive_buffer_size; // サイズを更新
        }
        // ------------------------------------------------

        // パケットの長さ(12バイト)以上貯まったら処理を開始する
        if(boss_receive_buffer_size >= 12)
        {
            ...
            if(recieve_cmd.is_exist)
            {
                ...
                // 次のコマンドに備えてバッファを完全に空にする
                boss_receive_buffer_size = 0;
                last_buffer_size = 0; // デバッグ変数もリセット
